...
export type CollectionConfigLike = {
  ...
  /**
   * Real Payload's own `auth` is `boolean | IncomingAuthType` - only
   * truthiness is read here, by `hashIncomingPassword` below (What's left
   * #6's fix: hashing a `users`-style collection's plain `password` field
   * into `salt`/`hash`). Typed `unknown` rather than `boolean | Record<
   * string, unknown>` (changed 2026-09-26, see `read-operations.ts`'s
   * `ReadEntityConfig.auth` doc comment for the two-directions-of-
   * assignability reason): `IncomingAuthType` has no index signature, so
   * `Record<string, unknown>` rejected a real `CollectionConfig`'s `auth`
   * value at the `readRegistry`/`engine.ts` boundary once `ReadEntityConfig`
   * gained the same field. `unknown` is the only type both directions
   * accept, and truthiness-only reads don't need anything narrower.
   */
  auth?: unknown
}
