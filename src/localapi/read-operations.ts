...
export type ReadEntityConfig = {
  ...
  /**
   * Mirrors `operations.ts`'s own `CollectionConfigLike.auth` - same real
   * property (`Users.ts` declares `auth: true`), just not previously part of
   * this narrower read-side type. Needed for `stripAuthFields` below.
   *
   ...
   * `field.hidden`). This function is the targeted stand-in for that missing
   * `hidden` mechanism, scoped to exactly the implicit auth columns rather
   * than a general `field.hidden` feature (lower risk - see incident log for
   * why a general fix wasn't attempted this session).
   */
  auth?: unknown
}
 ...
 * reset-token"), so this strips it unconditionally too, for consistency with
 ...
 * anyway (grepped - confirmed nothing does yet), but harmless either way.
 */
const SENSITIVE_AUTH_COLUMNS = [
  'salt',
  'hash',
  'resetPasswordToken',
...
/** Applied to every doc this module returns for an `auth: true` collection - see `ReadEntityConfig.auth`'s doc comment for why this can't just be field-level access. A no-op for every non-auth collection/global. */
function stripAuthFields(config: ReadEntityConfig, doc: Record<string, unknown>): void {
  if (!config.auth) return
  for (const key of SENSITIVE_AUTH_COLUMNS) delete doc[key]
}
...
async function findByIDInternal(entry: CollectionReadEntry, id: number, ctx: Omit<TraverseCtx, 'currentDepth'> & { currentDepth: number; disableErrors: boolean }): Promise<Doc | null> {
  ...
  stripAuthFields(entry.config, doc)
  return doc
}
...
export async function find(registry: ReadRegistry, collection: string, args: FindArgs): Promise<PaginatedDocs> {
  ...
  const docs = await Promise.all(
    result.docs.map(async (rawDoc) => {
      ...
      // See `findByIDInternal`'s matching comment - must run AFTER
      // `traverseFields`, not before, so `loginAttempts`'s real
      // `defaultValue: 0` backfill can't reintroduce it.
      stripAuthFields(entry.config, doc)
      return doc
    }),
  )
