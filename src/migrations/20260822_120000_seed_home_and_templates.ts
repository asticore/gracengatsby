...
// starter Page Templates, entirely through the Payload local API so it runs
// automatically on every deploy (`payload migrate`) without needing anyone
// to log into /admin first. Fully idempotent - safe to run against a
// database that already has a homepage and/or templates (it just skips).
        ...
        blocks: [
          {
            ...
            subheading: 'A curated boutique for the modern romantic - considered pieces, small-batch goods, and evenings worth dressing up for.',
    ...
    await payload.create({
      ...
      data: {
        ...
        blocks: [
          ...
          { blockType: 'ctaBanner', heading: 'Call to action', text: 'A reason to click the button below.', buttonLabel: 'Shop now', buttonUrl: '/shop', style: 'dark' },
        ],
      },
    })
    ...
    await payload.create({
      ...
      data: {
        ...
        blocks: [
          ...
          { blockType: 'richText', content: lexicalParagraph('Tell the story of the brand, the people behind it, and what makes it different.') },
