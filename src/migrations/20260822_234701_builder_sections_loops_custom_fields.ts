...
//
// The CLI's `migrate:create` produced a full-schema dump here rather than a diff,
...
// schema the CMS config wants and the schema that already exists: 24 new tables, 90
...
// in wrangler.jsonc for why the CLI's `migrate` cannot reach real D1 from CI).
//
...
//   1. the CLI's `migrate:create <name>` - emits a full-schema dump, not a diff.
...
export async function down({ payload: engine }: MigrateDownArgs): Promise<void> {
  ...
  engine.logger.info('[migrate] Down is a no-op - page-builder schema is left in place.')
}
