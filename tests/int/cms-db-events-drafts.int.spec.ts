...
describe('cms/db - events draft/publish policy (createDraftOps)', () => {
  ...
  it('Payload writes, ours reads: engine.create defaults to draft, ours sees it on both live and draft reads', async () => {
    ...
    // createEventLiveRow (plain baseOps.create, NOT the createDraftOps-wrapped
    // createEvent) writes only the live row, leaving Payload's own separate,
    ...
    const versions = await findEventVersions(id)
    expect(versions).toHaveLength(1)
  })
