...
import { createPost, deletePost, findPostByID, findPostVersions, updatePost } from '@/cms/db'
import { getDb } from '@/cms/db/connect'

// Posts' `content` field is required rich text - see tests/int/cms-db-posts.int.spec.ts's own richText helper, same shape.
const richText = (text: string) => ({ root: { children: [{ type: 'paragraph', children: [{ text }] }] } })

/**
 * The same createDraftOps policy proven against Events and Pages, wired onto
 * Posts - see ../../src/cms/db/collections/posts.ts and
 * ../../src/cms/db/generic.ts's createDraftOps doc comment. Posts has a
 * versioned array field (`categories`, proven in cms-db-posts.int.spec.ts)
 * but no join field, so - like Pages - this is a narrower proof: it confirms
 * the same policy holds on a third drafts-enabled collection, not a new
 * behaviour.
 *
 ...
 */
describe('cms/db - posts draft/publish policy (createDraftOps)', () => {
  let engine: Engine
  ...
  it('ours writes, Payload reads: create defaults to draft on both live and version', async () => {
    ...
    const ours = await createPost({ title: 'Ours: created draft', content: richText('Body.') })
    ...
    const versions = await findPostVersions(ours.id)
  ...
  it('ours writes, Payload reads: a normal (non-draft) update publishes live AND versions', async () => {
    const ours = await createPost({ title: 'Ours: pre-publish', content: richText('Body.') })
    createdIds.push(ours.id)

    const published = await updatePost(ours.id, { title: 'Ours: published', _status: 'published' })
  ...
  it('ours writes, Payload reads: draft:true leaves the live row untouched, versions only', async () => {
    const ours = await createPost({ title: 'Ours: base', content: richText('Body.') })
    createdIds.push(ours.id)
    await updatePost(ours.id, { title: 'Ours: published base', _status: 'published' })

    const draftEdit = await updatePost(ours.id, { title: 'Ours: draft on top' }, { draft: true })
    expect(draftEdit?.title).toBe('Ours: draft on top')

    const viaPayloadDefault = await engine.findByID({ collection: 'posts', id: ours.id, depth: 0 })
    expect(viaPayloadDefault.title).toBe('Ours: published base')
    expect((viaPayloadDefault as { _status?: string })._status).toBe('published')

    const oursDefault = await findPostByID(ours.id)
    expect(oursDefault?.title).toBe('Ours: published base')

    const viaPayloadDraft = await engine.findByID({ collection: 'posts', id: ours.id, depth: 0, draft: true })
    expect(viaPayloadDraft.title).toBe('Ours: draft on top')
    const oursDraft = await findPostByID(ours.id, { draft: true })
  ...
  it('Payload writes, ours reads: engine.create defaults to draft, ours sees it on both live and draft reads', async () => {
    const created = await engine.create({ collection: 'posts', data: { title: 'Payload: created draft', content: richText('Body.') } })
    const id = created.id as number
    createdIds.push(id)
    expect((created as { _status?: string })._status).toBe('draft')

    const oursDefault = await findPostByID(id)
  ...
  it('Payload writes, ours reads: a real Payload draft:true update leaves live untouched, ours agrees', async () => {
    const created = await engine.create({
      collection: 'posts',
      data: { title: 'Payload: base', content: richText('Body.'), _status: 'published' },
    })
    const id = created.id as number
    createdIds.push(id)

    await engine.update({ collection: 'posts', id, data: { title: 'Payload: draft on top' }, draft: true })
